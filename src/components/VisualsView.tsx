"use client";

import Image from "next/image";
import Link from "next/link";

const BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH ?? "";

// Tiny base64 blur-up placeholders, generated by scripts/convert-images.ts.
const BLUR = {
  "encryption-performance-comparison":
    "data:image/webp;base64,UklGRk4AAABXRUJQVlA4IEIAAACwAwCdASoUAA0APu1iqU2ppaOiMAgBMB2JZQC+SCHfJvY7WFlgAAD+7oTj/muSsZKKE/Lj1/rVNGR5isTVDM8AAAA=",
  "hashing-vs-encryption-vs-encoding":
    "data:image/webp;base64,UklGRkoAAABXRUJQVlA4ID4AAABQAwCdASoUAA0APu1iqU2ppaOiMAgBMB2JZQDCgDBRlfsBQAD+76VAGsSYpuKnFb2109u3JU/ZECASWZG4AA==",
  "tls-https-data-flow":
    "data:image/webp;base64,UklGRlIAAABXRUJQVlA4IEYAAABQAwCdASoUAA0APu1iqU2ppaOiMAgBMB2JZwDE2CzbpSP6AAD+7y5VWfqyvdTPPDX9Psr7XrDa0hWJgxR2udNM7+ypgAAA",
  "crypto-timeline":
    "data:image/webp;base64,UklGRlYAAABXRUJQVlA4IEoAAABQAwCdASoUAA0APu1iqk2ppaQiMAgBMB2JZQC2yCzcImV4AAD+74OYnjOhEZwReYp9k2vyMFQeLZnSLxP78tftZmo44bzbDVJAAA==",
} as const;

const VISUALS = [
  {
    src: `${BASE_PATH}/images/encryption-performance-comparison.webp`,
    blur: BLUR["encryption-performance-comparison"],
    alt: "Performance comparison of symmetric and asymmetric encryption algorithms across speed, key size, and security tradeoffs",
    heading: "Encryption Performance Comparison",
    desc: "Speed, key size, and security tradeoffs across the major algorithm families.",
  },
  {
    src: `${BASE_PATH}/images/hashing-vs-encryption-vs-encoding.webp`,
    blur: BLUR["hashing-vs-encryption-vs-encoding"],
    alt: "Side-by-side comparison of hashing, encryption, and encoding: their properties, reversibility, and use cases",
    heading: "Hashing vs Encryption vs Encoding",
    desc: "Three concepts that are frequently confused — each serves a distinct purpose in modern systems.",
  },
  {
    src: `${BASE_PATH}/images/tls-https-data-flow.webp`,
    blur: BLUR["tls-https-data-flow"],
    alt: "Diagram of TLS handshake and HTTPS data flow showing key exchange, authentication, and symmetric encryption in sequence",
    heading: "How TLS & HTTPS Work",
    desc: "Cryptography in action — how the algorithms on this site combine to secure every HTTPS connection.",
  },
  {
    src: `${BASE_PATH}/images/crypto-timeline.webp`,
    blur: BLUR["crypto-timeline"],
    alt: "Timeline of major cryptographic milestones from classical ciphers to post-quantum algorithms",
    heading: "Evolution of Cryptography",
    desc: "From Caesar ciphers to post-quantum standards — the history of the field at a glance.",
  },
];

export default function VisualsView() {
  return (
    <div
      style={{
        background: "var(--color-bg)",
        color: "var(--color-text)",
        minHeight: "100vh",
        fontFamily: "var(--font-ibm-plex-sans), 'IBM Plex Sans', -apple-system, sans-serif",
        lineHeight: 1.6,
      }}
    >
      <div
        style={{
          height: "3px",
          background:
            "linear-gradient(90deg, #3b82f6 0%, #06b6d4 12%, #8b5cf6 24%, #f59e0b 36%, #10b981 48%, #ec4899 60%, #ef4444 72%, #6366f1 84%, #3b82f6 100%)",
        }}
        aria-hidden="true"
      />

      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 24px" }}>
        {/* Header */}
        <header style={{ borderBottom: "1px solid var(--color-border)", padding: "20px 0" }}>
          <div
            style={{
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              flexWrap: "wrap",
              gap: "12px",
            }}
          >
            <Link
              href="/"
              style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: "10px" }}
            >
              <span
                style={{
                  fontSize: "22px",
                  fontWeight: 700,
                  fontFamily: "var(--font-jetbrains-mono), 'JetBrains Mono', monospace",
                  letterSpacing: "-0.5px",
                  color: "var(--color-text)",
                }}
              >
                <span style={{ color: "var(--color-accent-blue)" }}>◈ crypto</span>::compare
              </span>
            </Link>
            <Link
              href="/"
              style={{
                background: "var(--color-bg-control)",
                color: "var(--color-text-body)",
                border: "1px solid var(--color-border-muted)",
                borderRadius: "7px",
                padding: "10px 14px",
                fontSize: "13px",
                fontWeight: 600,
                textDecoration: "none",
                fontFamily: "var(--font-jetbrains-mono), 'JetBrains Mono', monospace",
                minHeight: "44px",
                display: "inline-flex",
                alignItems: "center",
              }}
            >
              ← Algorithm Reference
            </Link>
          </div>
          <div style={{ marginTop: "14px" }}>
            <h1
              style={{
                margin: "0 0 6px",
                fontSize: "26px",
                fontWeight: 700,
                fontFamily: "var(--font-jetbrains-mono), 'JetBrains Mono', monospace",
                letterSpacing: "-0.5px",
                color: "var(--color-text-heading)",
              }}
            >
              Visual Guide
            </h1>
            <p style={{ margin: 0, fontSize: "15px", color: "var(--color-text-muted)", lineHeight: 1.6 }}>
              Infographics and diagrams to help understand modern cryptography concepts.
            </p>
          </div>
        </header>

        {/* Images */}
        <main style={{ padding: "40px 0 64px" }}>
          {VISUALS.map((v) => (
            <section
              key={v.heading}
              style={{
                marginBottom: "48px",
                textAlign: "center",
              }}
            >
              <Image
                src={v.src}
                alt={v.alt}
                width={1440}
                height={810}
                placeholder="blur"
                blurDataURL={v.blur}
                sizes="(max-width: 768px) 100vw, 720px"
                style={{
                  display: "block",
                  width: "100%",
                  maxWidth: "720px",
                  height: "auto",
                  margin: "0 auto",
                  borderRadius: "10px",
                  boxShadow: "0 4px 32px var(--color-shadow-image-heavy), 0 1px 6px var(--color-shadow-image-soft)",
                }}
              />
              <p
                style={{
                  margin: "10px auto 0",
                  maxWidth: "560px",
                  fontSize: "13px",
                  color: "var(--color-text-dim)",
                  lineHeight: 1.6,
                }}
              >
                {v.desc}
              </p>
            </section>
          ))}
        </main>

        {/* Footer */}
        <div
          style={{
            height: "1px",
            background:
              "linear-gradient(90deg, transparent 0%, #3b82f640 20%, #8b5cf640 50%, #06b6d440 80%, transparent 100%)",
            marginBottom: "20px",
          }}
          aria-hidden="true"
        />
        <footer
          style={{
            paddingBottom: "28px",
            fontSize: "13px",
            color: "var(--color-text-label)",
            fontFamily: "var(--font-jetbrains-mono), 'JetBrains Mono', monospace",
            lineHeight: 1.7,
          }}
        >
          <div style={{ marginBottom: "6px" }}>
            <span style={{ color: "var(--color-accent-blue-label)", fontWeight: 700 }}>Dataset reviewed:</span>{" "}
            <time dateTime="2026-06-27">June 27, 2026</time>
          </div>
          Sources: NIST FIPS, IETF RFCs, KPQC, CRYPTREC, GB/T, GOST, DSTU, ISO, Eurocrypt/CRYPTO proceedings.
        </footer>
      </div>
    </div>
  );
}
