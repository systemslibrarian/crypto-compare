...
const SAFE_DEFAULTS: SafeDefault[] = [
  {
    ...
    why: "Memory-hard password hashing raises attacker cost materially after database compromise. Argon2id balances resistance to GPU cracking and side-channel leakage; the actual cost advantage depends on the parameters and attacker hardware.",
    ...
    wrongChoiceConsequence: "Using a fast hash such as SHA-256 or MD5 for password storage lets attackers test guesses far more cheaply than a properly tuned password hash after a database breach.",
  },
  {
  ...
  },
  {
    ...
    why: "Ed25519 provides strong signatures with deterministic nonce generation (avoiding failures caused by weak random ECDSA nonces), compact 64-byte signatures, and a mature ecosystem. For post-quantum planning, FIPS 204 assigns ML-DSA-65 to NIST security category 3; that category is not an exact bit-strength claim.",
  ...
  },
  {
    ...
    why: "SHA-256 is a broadly interoperable, NIST-standardized choice with hardware acceleration on many modern processors and extensive protocol support. BLAKE3's parallel design can be attractive when measured throughput on the target platform matters more than standards compatibility.",
    ...
    commonMistakes: [
      "Using SHA-256 directly for password storage — fast hashes let attackers test guesses far too cheaply.",
      "Assuming SHA-256 provides authentication — hashes do not authenticate. Use HMAC-SHA-256 for message authentication.",
      "Using bare SHA-256 as a keyed authenticator — its Merkle–Damgård construction permits length extension. Use HMAC or a purpose-built MAC.",
    ],
  ...
  },
  {
    ...
    why: "HMAC-SHA-256 is a widely interoperable keyed authenticator standardized in FIPS 198-1 and RFC 2104. Generic quantum search changes idealized brute-force estimates, but real assurance also depends on key strength, tag length, protocol use, and implementation.",
