...
98 algorithms. 17 categories. 192 unique linked public demos. Sourced recommendations. Safe-usage guidance. Reference architectures. Post-quantum migration context.
...
![TLS and HTTPS data flow visual](public/images/tls-https-data-flow.webp)
...
|--------|----------------|
| **98 algorithms across 17 categories** | Coverage is broad enough to support real design tradeoffs, not just a curated shortlist |
...
| **File / Object Encryption at Rest** | AES-256-GCM where hardware acceleration exists, otherwise XChaCha20-Poly1305; HKDF for key separation | AEAD prevents silent tampering; per-file derivation limits blast radius | Per-file encryption, backup encryption, encrypted archives, object storage | ECB, CBC without authentication, nonce reuse, same key for every purpose | Add PQ wrapping when long-term confidentiality matters; use a storage-specific design such as XTS-AES for full-disk encryption |
...
|-----------|--------------------|
| **Browse by category** | 98 algorithms across elliptic curves, encryption, KEM, signatures, hashing, password hashing, ZKPs, MPC, OT/PIR, threshold signatures, and more |
...
| `npm run test` | Run the automated unit/component test suite |
| `npm run test:e2e` | Run the Playwright end-to-end smoke tests against the static export |
